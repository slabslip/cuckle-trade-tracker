...
- A fifth tab, a hamburger as primary nav, or a + FAB as a tab. Apple allows 3–5 tabs;
  we stay at **four**: Home | Teams | Ledger | Data.
...
| **HIG-03** | Tab bar = 3–5 peer destinations | Four tabs only, bottom pill, icon **and** visible label. Most-used first (Home). Data's internal id stays `history`. |
| **HIG-04** | Selected tab is obvious without color alone | Active tab is a gold **oval** (shape) plus gold type. Ledger waiting is a gold **dot** plus the word Ledger in the accessible name (`N waiting`). |
...
| **HIG-08** | Do not hide the tab bar inside a tab | The pill stays up on Home / Teams / Ledger / Data. Sub-screens (`calc`, `news`, `cosmetics`, a seat) may hide it — they are modals / drill-ins, not tabs. Returning restores it. |
...
- Four cells, `repeat(4, minmax(0, 1fr))`, no wrap.
...
2. If it is a peer of Home, it is a **tab** — and we do not have a free tab.
3. If it is a task (calc, news, a trade), it is a **sub-screen**: hide the pill, keep Back.
