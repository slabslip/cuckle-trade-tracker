...
**Home overnight (v1):** one league letter each night, gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Not a first-person poke. Next need cut: hurt starter → cuff owner (or unowned), still on that card.
