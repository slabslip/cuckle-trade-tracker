...
**Home overnight (v1, Cuckle dynasty only):** league letter each night in dashboard chrome (Out / IR on the card; trades, wire, meters behind Show all), gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Off on GM / redraft. Out / IR rows name the starter’s cuff owner or `cuff unowned` from `cuffs.json`, and the collapsed list leads with Sunday starters, not book-value benches. **Team analyzer** on every dynasty team home uses dashboard chrome (same card as the Home letter), scores desk slots with `teamAnalyzerValueGrade` (`calcValueNum` vs `deskCuts`: starter 3 / stud 7.5 / elite 10) and weighs the next two drafts hardest on the pick chest (not vs the league; same bag reprints the same 0–10; team grade uses a window mix plus a fixed stretch so seats do not bunch at 6–7, a historic 2027/2028 chest moves a tank, and a thin chest does not sink a win-now roster). Now is this year’s starting desk (picks out); Later is the book plus the next two drafts. The gold chip saves a PNG. Next need cut: yesterday’s injury snap on the lede, still on that card.
