...
**Home overnight (v1, Cuckle dynasty only):** navy schematic league letter each night (settings grid + meters + titled lists), gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Off on GM / redraft. **Team analyzer** on every dynasty team home uses the same schematic and existing calculator + seat-direction books. Next need cut: hurt starter → cuff owner (or unowned), still on that card.
