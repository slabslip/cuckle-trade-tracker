...
**Home overnight (v1, Cuckle dynasty only):** league letter each night in dashboard chrome (meters + titled lists, no roster-slot grid), gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Off on GM / redraft. **Team analyzer** on every dynasty team home uses the navy schematic and existing calculator + seat-direction books. Next need cut: hurt starter → cuff owner (or unowned), still on that card.
