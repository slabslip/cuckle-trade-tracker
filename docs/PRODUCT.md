...
**Home overnight (v1, Cuckle dynasty only):** one league letter each night, gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Off on GM / redraft. Next need cut: hurt starter → cuff owner (or unowned), still on that card.
