...
**Home overnight (v1, Cuckle dynasty only):** league letter each night in dashboard chrome (Out / IR on the card; trades, wire, meters behind Show all), gold share chip, `?r=overnight` link — [`plans/dynasty_daily_habit.md`](./plans/dynasty_daily_habit.md). Off on GM / redraft. **Team analyzer** on every dynasty team home uses dashboard chrome (same card as the Home letter), scores desk slots and the top 12 picks with `teamAnalyzerValueGrade` (`calcValueNum` vs `deskCuts`: starter 3 / stud 7.5 / elite 10, not vs the league; same bag reprints the same 0–10; team grade is 70% position blend with a 30% weakest-slot pull / 15% depth / 15% draft), and the gold chip saves a PNG. Next need cut: hurt starter → cuff owner (or unowned), still on that card.
