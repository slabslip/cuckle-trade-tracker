...
**Status of this document.** The SQL in `db/schema.sql` was executed and tested
against a local PostgreSQL 16.15 instance, including the anon-role upsert path.
It has **not** been run against Supabase — nobody with access to your project ran
anything. The `curl` examples below are written from the Supabase and PostgREST
reference docs and have **not** been executed against a live project either.
Step 5 exists so you can confirm them yourself before we wire any UI.
...
## 6. Two read strategies, and which we recommend
...
Two values, in a DM or wherever:

1. **Project URL** — `https://<your-project-ref>.supabase.co`
2. **Publishable key** (`sb_publishable_...`) or legacy **`anon` key**
   (`eyJ...`) — please say which, since it changes the headers

Plus a yes/no on whether `curl` in Step 5 worked, and whether you are happy with
the limitation in Step 4 after reading it.

Do not send the secret / `service_role` key. We do not want it, cannot use it in
a static page, and it would have to be rotated.
