...
`history`). Law: [`DATA_SDD.md`](./DATA_SDD.md). Cold load is three panes: **Moves** (Give /
Get — five bands Hold → Blow), **League** (ten full-width cycle rows: Hard rebuild / Rebuild /
...
dropdowns; Tape has year chips. Moves peeks one idea per band (different asset ·
different team · why) with no bag totals. A row opens the calculator first-person
(A = you, B = them). The **trade block** is league-readable and owner-write on
