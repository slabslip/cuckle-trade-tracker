...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    a.champ-alert .champ-line { color: var(--text); font-weight: 650; margin-top: 12px; }
    a.champ-alert .date { margin-top: 2px; }
    button.day-in {
...
<body>
  ...
  <script>
    ...
    function daySides() {
      ...
      // "Today" is league.today, the day the book was priced -- not the browser's own
      // UTC date, which gave a viewer west of UTC "0 trades today" every evening. A deal
      // can land after the last price snapshot, so the newer of the two is the real today.
      const priced = (league && league.today) || "";
      const day = last > priced ? last : priced;
