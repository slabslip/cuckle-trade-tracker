  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* News and Alerts. The user asked for "scrolling", and this scrolls because a finger or a
       ...
       recorded as a WCAG 2.2.2 failure: a 48s loop with no pause control. That marquee has
       since been removed, and this box is now the reason the rule holds rather than the
       exception to it -- there is no self-animating region left anywhere on the page, and this
       is the region most likely to grow one. A news row is text a person needs time to read.
       So it stays a plain overflow box: capped height, newest at the top, and it stays where it
       is put. Nothing here needs a prefers-reduced-motion branch because nothing here moves. */
    .news-box {
...
<body>
  ...
  <script>
    ...
    let news = null;
    let lens = "all";
    const DATA_V = "20260830noticker1";
    /**
     ...
     * the trigger is always in view and the panel has just closed underneath it: focus moves to
     * the heading that names the set without moving the page. The "reveal" parameter that used
     * to scroll the heading into view existed for a ticker pill sitting well above the panel,
     * and it went with its one caller rather than staying as a branch nothing can take.
     */
...
// Exactly the five sets the five packs held. The ticker's pills used to name these ids too, so
// losing one left the other route standing; the dropdown is the only route now, and losing one
...
// blunt check has no branch that can be wrong, and prose has no reason to type `paintFeed`.
...
// 2. No marquee. The user asked for "scrolling" and the news box scrolls because a finger moves
//    it. This guard was written as "no *second* marquee" when #feed still looped on a 48s ticker
//    with no pause control -- the audit's WCAG 2.2.2 failure. That ticker has been removed, so
//    there is no first marquee either, and the whole-sheet assertion beside the ticker-removal
//    guard now covers the page. This one stays scoped to .news-box: it is the region a future
//    edit is most likely to animate, and a scoped guard names the right thing in its message.
...
// page. Both were verified to fire, separately, by breaking each region in turn.
//
