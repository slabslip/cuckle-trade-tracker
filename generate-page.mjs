  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .lh-hero.day-alert, .lh-hero.news-hero {
      ...
      /* Same dark plane as the left half — no right-side glow or split panel. */
      background: #1a1810;
    ...
    .lh-hero .day-alert-top .day-alert-h { min-width: 0; flex: 1 1 auto; }
    button.news-hero-pause {
      ...
      height: 26px; padding: 0 10px; cursor: pointer; touch-action: manipulation;
    }
...
<body>
  ...
  <script>
    ...
    let lens = "all";
    const DATA_V = "tradeNoAt20260901012300";
    /**
    ...
    /** Compact one-line copy for the rotating hero chip. */
    ...
    function latestTradeCardHtml(latest) {
      ...
      const sideHtml = (side, rightAlign) => {
        ...
        // Seat name only — @handle duplicated the same string under the title.
        return '<div class="' + cls + '">'
          + '<div class="h2h-top">'
    ...
    document.getElementById("app").addEventListener("click", (e) => {
      ...
      const newsPause = e.target.closest("[data-news-pause]");
      if (newsPause) {
        newsHeroPaused = !newsHeroPaused;
        armNewsHero();
        // Re-render so the Pause/Play label stays honest after the next paint;
        // for now update the control in place so the carousel does not restart from zero.
        newsPause.setAttribute("aria-pressed", newsHeroPaused ? "true" : "false");
        newsPause.textContent = newsHeroPaused ? "Play" : "Pause";
        return;
      }
...
if (!inline.includes("function armNewsHero()") || !inline.includes("data-news-pause")) {
  throw new Error("the News hero carousel must ship with a Pause control (WCAG 2.2.2)");
}
