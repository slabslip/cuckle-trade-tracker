  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body.has-news-pullup {
      padding-bottom: calc(var(--news-pullup-peek, 112px) + env(safe-area-inset-bottom, 0px));
    }
    ...
    .news-pullup-title-row .day-alert-h { min-width: 0; flex: 1 1 auto; }
    .news-pullup-count {
      flex: 0 0 auto; font-size: 0.75rem; color: var(--dim);
    }
    button.news-pullup-peek {
      flex: 0 0 auto;
      appearance: none; font: inherit; color: inherit; text-align: left;
      background: transparent; border: 0; margin: 0;
      padding: 4px 16px 14px; cursor: pointer;
      touch-action: none; user-select: none;
      min-height: 0;
    }
    button.news-pullup-peek:focus-visible { outline: 2px solid #c8c8d0; outline-offset: 2px; }
    button.news-pullup-peek[hidden] { display: none; }
    .news-pullup-panel {
    ...
    .news-pullup-row:first-child { border-top: 0; padding-top: 4px; }
    .news-pullup-empty {
      padding: 12px 0; color: var(--muted); font-size: 0.875rem; line-height: 1.4;
    }
    /* Peek / row stack: seat tag → summary → source underneath. */
    .news-hero-who {
    ...
    /* Year scope under each column lab — stacked so · N stay centered and
       names keep room (clearer than a cramped 1sts · 2027–2029 line). */
...
<body>
  ...
  <script>
    ...
    /** Compact year scope for column labs — all still-available seasons (e.g. 2027–2029). */
    ...
    function pickIntelBoard() {
      ...
      return '<div class="pick-intel-board" role="group" aria-label="' + esc(aria) + '">'
        ...
        + cols.map((r) => {
          ...
          const labFull = yrs ? (r.lab + " · " + yrs) : r.lab;
          ...
          const lab = r.filter
            ? ('<button type="button" class="pick-intel-board-lab" data-pick-round="'
              + r.round + '" aria-label="Filter to ' + esc(labFull) + '">' + labInner + "</button>")
            : ('<span class="pick-intel-board-lab">' + labInner + "</span>");
    ...
    function renderLeagueHome() {
      ...
      if (!hero) {
        hero = '<aside class="news-pullup" id="newsPullup" aria-label="News Feed"'
          ...
          + '<button type="button" class="news-pullup-peek" data-news-pullup-peek'
    ...
    function setNewsPullupOpen(open) {
      ...
      if (peek) peek.hidden = newsPullupOpen;
    ...
    function newsPullupItemHtml(bit) {
      return (bit.who ? '<div class="news-hero-who">' + bit.who + "</div>" : "")
        + '<div class="news-pullup-line">' + esc(bit.line) + "</div>"
    ...
    function latestTradeSumHtml(legs, tone) {
    ...
    document.getElementById("app").addEventListener("click", (e) => {
      ...
      if (pickRound) {
        ...
        // Leaderboard shortcut: jump straight into the year step with that round on.
        pickFilterRounds = {};
...
if (!html.includes("button.news-pullup-peek") || !html.includes(".news-pullup-sheet {")) {
...
if (!inline.includes("function newsHeroLine(") || !inline.includes("it.tweet_handle")) {
  throw new Error("newsHeroLine must append tweet_handle (@author) on the source line");
}
...
if (!html.includes(".news-hero-who") || !html.includes(".news-hero-src")) {
  throw new Error("News Feed must style seat tags (.news-hero-who) and source under summary (.news-hero-src)");
}
{
  const itemHtml = inline.slice(inline.indexOf("function newsPullupItemHtml("),
    inline.indexOf("function newsPullupItemHtml(") + 500);
  if (!(itemHtml.indexOf("news-hero-who") < itemHtml.indexOf("news-pullup-line")
      && itemHtml.indexOf("news-pullup-line") < itemHtml.indexOf("news-hero-src"))) {
    throw new Error("News pull-up item must order: seat tag, summary, then source underneath");
  }
}
...
if (!inline.includes("function pickIntel()") || !inline.includes('data-pick-mine="1"')
  || !inline.includes('data-pick-round') || !inline.includes('data-pick-step-round')
