...
<body>
  ...
  <script>
    ...
    async function authSignUp(username, password, sleeperUserId, espnUserId) {
      const name = normalizeUsername(username);
      if (!/^[A-Za-z0-9_][A-Za-z0-9_.-]{2,31}$/.test(name)) {
        throw new Error("Username: 3–32 chars, letters/numbers/_ . -");
      }
      if (String(password || "").length < 6) throw new Error("Password must be at least 6 characters.");
      const sleeperId = String(sleeperUserId || "").trim() || null;
      const espnId = String(espnUserId || "").trim() || null;
      if (!sleeperId && !espnId) {
        throw new Error("Enter your Sleeper user ID, ESPN user ID, or both.");
      }
      if (sleeperId && !/^\d{6,64}$/.test(sleeperId)) {
        throw new Error("Sleeper user ID should be digits (from your Sleeper profile URL).");
      }
      if (espnId && !/^[A-Za-z0-9_-]{3,64}$/.test(espnId)) {
        throw new Error("ESPN user ID looks off — use the ID from your ESPN fantasy profile.");
      }
      ...
      // Profile row — username + platform IDs; uniqueness enforced in DB.
      ...
      if (!prof.ok) {
        ...
        if (t.indexOf("app_profiles_sleeper_user") >= 0) throw new Error("That Sleeper user ID is already linked to an account.");
        if (t.indexOf("app_profiles_espn_user") >= 0) throw new Error("That ESPN user ID is already linked to an account.");
        if (t.indexOf("app_profiles_platform_required") >= 0) {
          throw new Error("Enter your Sleeper user ID, ESPN user ID, or both.");
        }
        throw new Error("Could not save profile (" + prof.status + ").");
      }
      authSave({
    ...
    async function authSignIn(username, password) {
      ...
      try {
        const pr = await fetch(
          VOTE_API + "/app_profiles?select=username,sleeper_user_id,espn_user_id&auth_user_id=eq."
        ...
        if (pr.ok) {
          ...
          if (rows[0]) {
            if (rows[0].username) uname = rows[0].username;
            sleeperId = rows[0].sleeper_user_id || null;
            espnId = rows[0].espn_user_id || null;
          }
        }
    ...
    async function onGateSubmit() {
      ...
      const sleeperEl = document.getElementById("gateSleeper");
      const espnEl = document.getElementById("gateEspn");
      const username = userEl && userEl.value;
      const password = passEl && passEl.value;
      const sleeperId = sleeperEl && sleeperEl.value;
      const espnId = espnEl && espnEl.value;
      authBusy = true;
      authError = "";
      render();
      try {
        if (gateMode === "signup") await authSignUp(username, password, sleeperId, espnId);
    ...
    async function onJoinPreview() {
      ...
      try {
        ...
        // Auto-match seat from the Sleeper user ID collected at signup.
        const mine = authSession && authSession.sleeper_user_id;
        if (mine && (joinPreview.teams || []).some((t) => t.sleeper_user_id === mine)) {
          joinSeatId = mine;
        } else if (mine) {
          joinError = "Your Sleeper user ID is not a seat in this league. Check the ID or pick your team below.";
        } else {
          joinError = "No Sleeper user ID on your account — pick your team below, or add a Sleeper ID under a new signup.";
        }
      } catch (err) {
    ...
    function votePush(transactionId, choice, seat) {
      ...
      fetch(VOTE_API + "/trade_votes?on_conflict=transaction_id,voter", {
        ...
        body: JSON.stringify({ transaction_id: transactionId, choice: sent, voter: seat }),
        signal: voteAbort(),
      }).then((res) => {
    ...
    function renderAppGate() {
      const title = gateMode === "signup" ? "Get started" : "Sign in";
      const go = gateMode === "signup" ? "Create account" : "Sign in";
      const platformFields = gateMode === "signup"
        ? '<p class="caption" style="margin:0">Required: your <b>Sleeper user ID</b>, <b>ESPN user ID</b>, or both if you moved leagues and still have data on each.</p>'
          + '<label>Sleeper user ID<input id="gateSleeper" name="sleeperId" inputmode="numeric" autocomplete="off"'
          + ' placeholder="Digits from sleeper.app" autocapitalize="off"'
          + (authBusy ? " disabled" : "") + " /></label>"
          + '<label>ESPN user ID<input id="gateEspn" name="espnId" autocomplete="off"'
          + ' placeholder="Optional if you entered Sleeper" autocapitalize="off"'
          + (authBusy ? " disabled" : "") + " /></label>"
        : "";
      return '<div class="app-shell">'
        + '<h2 class="screen-h" tabindex="-1">Chuckle Fantasy</h2>'
        + '<p class="caption">Trade meter for fantasy leagues. Link your Sleeper and/or ESPN ID, join a league by ID, then open your dashboard.</p>'
        ...
        + (gateMode === "signup"
          ...
          : 'New here? <button type="button" class="linkish" data-gate-mode="signup">Get started</button>')
        + "</p></div>"
        + '<p class="caption">CuckleChunckle managers: after you create an account, add league ID '
        + "<code>" + CUCKLE_LEAGUE_ID + "</code> to open the dashboard.</p>"
        + "</div>";
    }
    ...
    function renderAppHome() {
      ...
      const plat = [];
      if (authSession && authSession.sleeper_user_id) plat.push("Sleeper " + authSession.sleeper_user_id);
      if (authSession && authSession.espn_user_id) plat.push("ESPN " + authSession.espn_user_id);
      ...
      return '<div class="app-shell">'
        ...
        + (plat.length ? " · " + esc(plat.join(" · ")) : "")
        + ". "
        + '<button type="button" class="linkish" data-auth-signout="1">Sign out</button></p>'
        + (rows || '<p class="caption">No leagues yet. For CuckleChunckle, add Sleeper league ID '
          + "<code>" + CUCKLE_LEAGUE_ID + "</code>.</p>")
    ...
    function renderJoinLeague() {
      ...
      return '<div class="app-shell">'
        ...
        + '<p class="caption">Enter the Sleeper <b>league</b> ID. We match your seat from the Sleeper <b>user</b> ID on your account'
        + (mine ? " (<code>" + esc(mine) + "</code>)" : "")
        + ". CuckleChunckle uses <code>" + CUCKLE_LEAGUE_ID + "</code>.</p>"
        ...
        + (joinPreview
          ? '<div class="app-card"><h3>' + esc(joinPreview.name) + "</h3>"
            ...
            + (joinPreview.total_rosters || teams.length) + " teams"
            + (matched ? " · matched to your Sleeper ID" : "")
            + "</p>"
