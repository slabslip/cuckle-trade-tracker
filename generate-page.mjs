  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body.has-news-pullup {
      padding-bottom: calc(var(--news-pullup-peek, 148px) + env(safe-area-inset-bottom, 0px));
    }
    ...
    /* Peek / row stack: seat tag + source bubble → summary → account + time (+ link). */
    .news-hero-head {
    ...
    .news-hero-src-bubble {
      ...
      color: #d4c48a; background: rgba(107, 90, 46, 0.32);
      border: 1px solid rgba(107, 90, 46, 0.55);
      border-radius: 999px; padding: 3px 8px;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    .news-pullup-line {
      display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 4;
      overflow: hidden; color: var(--text); font-size: 0.8125rem; line-height: 1.38;
      margin: 0 0 8px;
    }
    ...
    .news-hero-foot {
      ...
      gap: 4px 10px; min-height: 1.1rem;
      font-size: 0.6875rem; line-height: 1.3; color: var(--dim);
    }
    .news-pullup-peek-block .news-hero-foot { padding-right: 4.75rem; }
    .news-hero-handle {
      font-weight: 600; color: #a8a8b0;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    .news-hero-when { color: var(--dim); white-space: nowrap; }
    a.news-hero-link {
      font-size: 0.6875rem; font-weight: 650; line-height: 1.3;
      color: #e0b44c; text-decoration: none; white-space: nowrap;
    }
    a.news-hero-link:hover, a.news-hero-link:focus-visible { text-decoration: underline; }
    .news-pullup-peek-block > a.news-hero-link {
      position: absolute; right: 16px; bottom: 14px;
    }
    .news-pullup-row a.news-hero-link { margin-left: auto; }
...
<body>
  ...
  <script>
    ...
    function newsHeroLine(it) {
      ...
      return { source: source, line: line, who: who, handle: handleLab, when: when, postUrl: postUrl };
    }

    function newsPullupLinkHtml(bit) {
      if (!bit.postUrl) return "";
      return '<a class="news-hero-link" href="' + esc(bit.postUrl)
        + '" target="_blank" rel="noopener noreferrer">See post</a>';
    }
    ...
    function newsPullupItemInnerHtml(bit) {
      ...
      const foot = footBits.length
        ? '<div class="news-hero-foot">' + footBits.join("") + "</div>"
        : "";
      return head + summary + foot;
    }

    function newsPullupItemHtml(bit) {
      return newsPullupItemInnerHtml(bit) + newsPullupLinkHtml(bit);
    }
    ...
    function dayAlert() {
      ...
      const latestBit = items.length
        ...
        : { source: "News", line: emptyLine, who: "", handle: "", when: "", postUrl: "" };
...
if (!html.includes(".news-hero-who") || !html.includes(".news-hero-src-bubble")
  ...
  throw new Error("News Feed must style seat tag, source bubble, foot row, and post link");
}
{
  ...
  if (!(itemHtml.indexOf("news-hero-head") >= 0
      ...
      && itemHtml.indexOf("news-hero-foot") > itemHtml.indexOf("news-pullup-line"))) {
    throw new Error("News pull-up item must order: head (seat + bubble), summary, then foot row");
  }
}
