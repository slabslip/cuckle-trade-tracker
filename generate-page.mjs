...
<body>
  <h1><a href="./">CuckleChunckle</a></h1>
  <p id="lead"></p>
  <select class="who" id="who" aria-label="Team">
    <option value="">League boards</option>
  </select>
  ...
  <script>
    ...
    function paintWho() {
      ...
      sel.innerHTML = '<option value="">League boards</option>'
        + members.map((m) =>
    ...
    function renderPlayerLists() {
      ...
      return "<h2>Most passed around</h2>" + most
        + "<h2>Least traded</h2>" + least
        + "<h2>Forever players</h2>" + forever
        + "<h2>Homesteaders</h2>" + stay;
    }

    function renderLeagueHome() {
      return "<h2>Best aged</h2>" + rankAge(1).map(boardTape).join("")
        + "<h2>Worst aged</h2>" + rankAge(-1).map(boardTape).join("")
        + renderPlayerLists();
    }
