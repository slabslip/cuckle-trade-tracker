...
<body>
  ...
  <script>
    ...
    const DATA_V = "news20260903093909";
    /**
     ...
     * Opens their team home (marks / best-worst / partners). Falls back to My Trades
     * when the session has no claimed seat yet.
     */
    function lhSeatStatsAction() {
      ...
      // data-who → selectMe (team home). Same destination as tapping the seat name elsewhere.
      return '<button type="button" class="lh-action" data-who="' + esc(name) + '"'
        + ' aria-label="' + esc(lab) + '">'
        + ico
        + '<span class="lh-lab">' + esc(lab) + "</span></button>";
    }
...
}
{
  ...
  if (!fn.includes(' + " stats"') || !fn.includes("lh-seat-flair")
    || !fn.includes("authSeatCanonName(") || !fn.includes('data-who="')) {
    throw new Error("lhSeatStatsAction must paint seat flair and '{team} stats' opening that seat");
  }
}
