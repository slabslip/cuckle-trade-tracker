...
<body>
  ...
  <script>
    ...
    async function loadMembers() {
      members = await (await fetch("data/ui/members.json")).json();
      document.getElementById("who").innerHTML = members.map((m) =>
        '<button type="button" class="tile" data-id="' + m.user_id + '">' + m.name + "</button>"
      ).join("");
      const hit = members.find((m) => m.name === startMe || m.user_id === startMe);
      if (hit) await selectMe(hit.user_id);
    }

    async function selectMe(id) {
      me = members.find((m) => m.user_id === id);
      data = await (await fetch("data/ui/me/" + id + ".json")).json();
      if (!league) league = await (await fetch("data/ui/league.json")).json();
      if (!picks) picks = await (await fetch("data/ui/picks.json")).json();
      history.replaceState(null, "", "?me=" + encodeURIComponent(me.name));
      document.querySelectorAll("#who .tile").forEach((b) => b.classList.toggle("on", b.dataset.id === id));
      document.getElementById("lead").textContent = me.name + " · DynastyProcess Superflex · FAAB thrown out";
    ...
    function sideOf(t) {
      ...
      if (lens === "y3") return t.y3 || t.realized;
      return t.realized;
    }

    function renderHome() {
      const h = data.hero || {};
      const per = lens === "pick" ? h.pick_per_trade : h.realized_per_trade;
      const total = lens === "pick" ? h.pick_total : h.realized_total;
    ...
    function tradeRow(t) {
      ...
      let detail = "";
      if (open) {
        let bags = bagBlock("You received", s.legs, s.today, s.unpriced)
          + bagBlock("You gave up", s.sent, s.sent_today, s.sent_unpriced);
        if ((t.others || []).length > 1) {
          bags += (t.other_bags || []).map((b) => {
            const side = lens === "pick" ? b.pick : lens === "y3" ? (b.y3 || b.realized) : b.realized;
            return bagBlock(b.name + " received", side.legs, side.today, side.unpriced);
          }).join("");
        }
        const signed = (n) => (n > 0 ? "+" : "") + fmt(n);
        const aged = lens === "realized" && s.t0_delta != null
        ...
        const clockNote = lens === "pick"
          ...
          : lens === "y3"
          ? "Average of year-end values in the 3 years after accept. Under 300 counts as 0 (retired)."
          : "Numbers are that player today. Name in parentheses drafted the pick, not who got it in this trade.";
        const line = spark((lens === "pick" ? t.pick_year_ends : t.year_ends).map((p) => ({ as_of: p.as_of, ...p.points })));
        const hint = line
          ? '<p class="caption">Each line = that side received bag at year-end · Number on the row = received minus sent'
            + (lens === "y3" ? " (first 3 years)" : " today") + "</p>"
          : "";
        detail = '<div class="detail"><div class="bags">' + bags + "</div>"
          + aged + '<p class="caption">' + clockNote + "</p>" + line + hint + "</div>";
      }
      const others = t.others.join(" · ");
      return '<button type="button" class="row' + (open ? " open" : "") + '" data-id="' + t.transaction_id + '">'
        + '<div class="row-top"><div><div class="names">' + others + "</div>"
    ...
    function renderLeague() {
      ...
      const traders = (league.traders || []).map((t) =>
        ...
        + '<div class="margin ' + cls(t.realized_per_trade) + '">' + fmt(t.realized_per_trade) + " / trade</div></div>"
    ...
    function renderTradeBoards() {
      ...
      return '<p class="caption">Complete 2-team only. Became the player. As of today = received minus sent now. Aged = that minus the same at accept. Window = trade date.</p>'
        + '<div class="toggle">'
    ...
    function renderPartners() {
      ...
      const perOf = (p) => lens === "pick" ? p.pick_per_trade : p.realized_per_trade;
      ...
      if (partnerName) {
        ...
        detail = p ? "<h2>" + p.name + "</h2>"
          + '<p class="caption">' + gradeLabel(lens === "pick"
            ...
            : p.grade)
          + " · " + fmt(lens === "pick" ? p.pick_per_trade : p.realized_per_trade) + " per complete trade"
          + " · total " + fmt(lens === "pick" ? p.pick_total : p.realized_total)
    ...
    function render() {
      ...
      const nav = '<div class="nav">'
        + ["home","trades","partners","drafts","league"].map((v) =>
          '<button type="button" class="tab' + (view === v ? " on" : "") + '" data-view="' + v + '">' + v + "</button>"
        ).join("")
        + "</div>"
        + '<div class="toggle">'
        + '<button type="button" class="chip' + (lens === "realized" ? " on" : "") + '" data-lens="realized">Became the player</button>'
        ...
        + "</div>"
        + (lens === "y3" ? '<p class="caption">Average of year-end values in the 3 years after accept. Under 300 counts as 0 (retired).</p>' : "");
      const body = view === "home" ? renderHome() : view === "trades" ? renderTrades()
