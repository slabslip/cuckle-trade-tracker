...
<body>
  ...
  <script>
    ...
    function sideOf(t) {
      ...
      if (lens === "even") return t.even || t.realized;
      return t.realized;
    }
    ...
    function renderHome() {
      ...
      const per = lens === "pick" ? h.pick_per_trade
        : lens === "even" ? (h.even_per_trade ?? h.realized_per_trade)
        : h.realized_per_trade;
      const total = lens === "pick" ? h.pick_total
        : lens === "even" ? (h.even_total ?? h.realized_total)
        : h.realized_total;
    ...
    function tradeRow(t, extra) {
      ...
      if (open) {
        ...
        if ((t.others || []).length > 1) {
          bags += (t.other_bags || []).map((b) => {
            const side = lens === "pick" ? b.pick
              : lens === "y3" ? (b.y3 || b.realized)
              : lens === "even" ? (b.even || b.realized)
              : b.realized;
            return bagBlock(b.name + " received", side.legs, side.today, side.unpriced);
          }).join("");
        }
        const signed = (n) => (n > 0 ? "+" : "") + fmt(n);
        const aged = (lens === "realized" || lens === "even") && s.t0_delta != null
        ...
        const clockNote = lens === "pick"
          ...
          : lens === "even"
          ? "40% even-flatten DP + 60% KTC Superflex on dates we snapped. Older year-ends are flatten-only (no 2019–2025 KTC). KTC is not re-flattened."
          : "Steep DynastyProcess today. Name in parentheses drafted the pick, not who got it in this trade.";
        const sparkSrc = lens === "pick" ? t.pick_year_ends : lens === "even" ? (t.even_year_ends || t.year_ends) : t.year_ends;
        ...
        const hint = line
          ? '<p class="caption">Each line = that side received bag at year-end · Number on the row = received minus sent'
            + (lens === "y3" ? " (first 3 years)" : lens === "even" ? " (60% KTC + even-DP)" : " today") + "</p>"
          : "";
        const steepNote = extra && extra.steep != null && lens === "even"
          ? '<p class="caption">Steep DP (Became the player): ' + (extra.steep > 0 ? "+" : "") + fmt(extra.steep)
    ...
    function renderPartners() {
      ...
      const perOf = (p) => lens === "pick" ? p.pick_per_trade
        : lens === "even" ? (p.even_per_trade ?? p.realized_per_trade)
        : p.realized_per_trade;
      ...
      if (partnerName) {
        ...
        detail = p ? "<h2>" + p.name + "</h2>"
          + '<p class="caption">' + gradeLabel(lens === "pick"
            ...
            : p.grade)
          + " · " + fmt(lens === "pick" ? p.pick_per_trade : lens === "even" ? (p.even_per_trade ?? p.realized_per_trade) : p.realized_per_trade) + " per complete trade"
          + " · total " + fmt(lens === "pick" ? p.pick_total : lens === "even" ? (p.even_total ?? p.realized_total) : p.realized_total)
    ...
    function render() {
      ...
      const nav = '<div class="nav">'
        ...
        + '<button type="button" class="chip' + (lens === "even" ? " on" : "") + '" data-lens="even">KTC blend</button>'
        + "</div>"
        + (lens === "y3" ? '<p class="caption">First 3 years: flatten each year-end (player years under 300 raw DP → 0). KTC 60% mixes in only on snapped dates. Home hero follows the active chip.</p>' : "")
        + (lens === "even" ? '<p class="caption">Default book: 40% even-flatten DynastyProcess + 60% KTC Superflex. KTC only on weeks we snapped. Became the player is still raw steep DP.</p>' : "");
