 ...
 * Redraft / GM passes a 3rd-place-game + RS-last-place rule in. One book also powers
...
import { PLAYOFF_AVG_FLOOR, careerNetFromParts, madePlayoffs, payoutForPlace } from "./redraft-season.mjs";
...
/** Regular-season points only. Hunt / consolation PF must not win MP. */
export function regularSeasonPointsForMp(row) {
  if (row && row.rs_fpts != null && Number.isFinite(Number(row.rs_fpts))) return Number(row.rs_fpts);
  if (row && row.fpts_from === "regular+hunt") return null;
...
function mostPointsBySeason(seats) {
  ...
  for (const seat of seats || []) {
    for (const row of seat.places || []) {
      const year = String(row.season || "");
      const pts = regularSeasonPointsForMp(row);
...
function decorateSeat(seat, lastByYear, pot, mpBySeason) {
  ...
  const sackoFee = pot && pot.sacko != null ? Number(pot.sacko) : 0;
  if (pot && pot.entry && sackoFee) {
    for (const year of lastYears) {
      const row = places.find((p) => String(p.season) === String(year));
      payouts.push({
        season: year,
        place: row && row.place != null ? Number(row.place) : lastByYear[year],
        amount: sackoFee,
        kind: "sacko",
      });
    }
  }
  const lost = pot && pot.entry
    ? n * Number(pot.entry) + lastN * (Number.isFinite(sackoFee) ? sackoFee : 0)
    : null;
  return {
    ...seat,
    ...
    net: pot && lost != null ? careerNetFromParts(won, lost) : null,
    payouts: pot ? payouts : [],
  };
}
...
/** One row per completed season: that year's prizes minus entry (and sacko). */
export function seatYearLedger(seat, pot) {
  const entry = pot && pot.entry != null ? Number(pot.entry) : 0;
  const baseLost = Number.isFinite(entry) ? entry : 0;
  const wonBy = {};
  const sackoBy = {};
  const kindsBy = {};
  for (const pay of (seat && seat.payouts) || []) {
    ...
    if (!year) continue;
    if ((pay && pay.kind) === "sacko") {
      sackoBy[year] = (sackoBy[year] || 0) + (Number(pay.amount) || 0);
    } else {
  ...
  return ((seat && seat.places) || []).map((row) => {
    ...
    const lost = baseLost + (sackoBy[year] || 0);
    return {
      season: year,
...
/** Concrete 1–N board for every completed year. Places stay; leftover scores do not live here. */
export function yearStandingsFromSeats(seats) {
  const byYear = {};
  for (const seat of seats || []) {
    for (const row of seat.places || []) {
      const year = String((row && row.season) || "");
      const place = Number(row && row.place);
      if (!year || !Number.isFinite(place) || place < 1) continue;
      (byYear[year] || (byYear[year] = [])).push({
        place,
        rs_place: row.rs_place != null ? Number(row.rs_place) : null,
        from: row.from || null,
        user_id: seat.user_id,
        name: seat.name,
        wins: row.wins != null ? Number(row.wins) : null,
        losses: row.losses != null ? Number(row.losses) : null,
        ties: row.ties != null ? Number(row.ties) : 0,
        fpts: row.fpts != null ? Number(row.fpts) : null,
        rs_fpts: row.rs_fpts != null ? Number(row.rs_fpts) : null,
        provider: row.provider || null,
      });
    }
  }
  return Object.keys(byYear).sort((a, b) => b.localeCompare(a)).map((season) => {
    const rows = byYear[season].slice().sort((a, b) => a.place - b.place);
    return { season, n: rows.length, rows };
  });
}
...
export function buildFinishesBook({
  ...
  return {
    ...
    n: seats.length,
    seasons,
    years: yearStandingsFromSeats(seats),
    rule,
