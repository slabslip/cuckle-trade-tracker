 ...
 *
 * Final standings each year stay concrete. Only leftover scores — weeks
 * where a seat is out of money and stops setting a lineup — drop from
 * point averages. Places never come from consolation reshuffles.
 *
 * Places (top six from the playoff bracket, bottom six from RS):
 *   1–2  championship game (title hunt)
 *   3–4  3rd-place game (semifinal losers)
 *   5–6  first-round losers, ordered by regular-season record
 *   7–N  missed playoffs, ordered by regular-season record
 *        last place = worst regular-season record (sacko)
 *
 * Points: regular-season weeks plus money playoff weeks (title path and the
 * 3rd-place game — those seats set lineups). Stubs under 20, 5th-place, and
 * losers consolation are dropped.
 */
import { scoreIsChampionshipHunt, scoreIsSemiMoneyPlace, titlePathWeeks } from "./week-score-lists.mjs";

export const PLAYOFF_N = 6;
export const PLAYOFF_AVG_FLOOR = 2;
export const REDRAFT_PLACE_RULE =
  "title hunt 1-2; 3rd-place game 3-4; first-round outs and missed playoffs by regular season; last = worst RS record; consolation points out";
...
export function seasonPointBuckets(scores, season) {
  ...
  // 3rd-place game is in the money — those two semi losers set lineups.
  // Do not move lastHunt; title-path weeks still decide exit groups.
  const path = titlePathWeeks(scores, year);
  for (const score of scores || []) {
    if (!score || String(score.season || "") !== year) continue;
    if (!scoreIsSemiMoneyPlace(score, scores, path)) continue;
    const uid = score.user_id == null ? "" : String(score.user_id);
    if (!uid || !by[uid]) continue;
    const pts = Number(score.points);
    const add = Number.isFinite(pts) ? pts : 0;
    by[uid].hunt += add;
    by[uid].cons = Math.max(0, (by[uid].cons || 0) - add);
  }
  return by;
}
...
export function isThirdPlaceScore(score, finalWeek) {
  ...
  const pts = Number(score.points);
  if (Number.isFinite(pts) && pts < 20) return false;
 ...
 */
export function thirdPlaceGameOrder(semis, scores, season, finalWeek, buckets) {
  ...
  const ranked = scored.slice().sort((a, b) => {
    ...
    if (pb !== pa) return pb - pa;
    return regularSeasonCompare(a, b, buckets);
  });
...
function orderSemis(semis, buckets, scores, season, finalWeek) {
  return thirdPlaceGameOrder(semis, scores, season, finalWeek, buckets)
 ...
 * money-game points (title + 3rd). `champUserId` is the real champion.
 */
