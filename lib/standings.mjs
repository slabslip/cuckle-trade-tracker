...
/**
 * Championship hunt plus the 3rd-place game. p=1 is 1st/2nd. p=3 is 3rd/4th.
 * Those money games set lineups. p=5 (5th-place / consolation) is ignored.
 * First-round losers (r=1, no p) keep the place their regular-season record
 * earns inside that exit group.
 */
export function huntExitFromBracket(wb) {
  ...
  for (const row of wb || []) {
    ...
    if (place === 3) {
      if (row.w) third = row.w;
      if (row.l) fourth = row.l;
      continue;
    }
  ...
  return { champ, second, third, fourth, firstOut, semiOut };
}
...
export function standingsForRedraft(s, nameByUser) {
  ...
  rows.slice().sort(cmp).forEach((r, i) => {
    r.rs_place = i + 1;
    r.rs_fpts = r.fpts;
  });
  ...
  if (exits.third) take(exits.third, "semi");
  if (exits.fourth) take(exits.fourth, "semi");
