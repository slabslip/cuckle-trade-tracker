...
/**
 * Title-path rosters by NFL week from a Sleeper winners_bracket.
 * Hunt is still-alive (`p` empty) or the title game (`p === 1`) only.
 * `p === 3` is a money lineup counted later — tagging it hunt would
 * move lastHunt to the championship week and promote semis to finalists.
 * `p === 5` is out.
 */
...
export function scoreIsChampionshipHunt(score) {
  ...
  // 3rd-place / losers consolation must never move lastHunt, even if a
  // rebuild mis-tags the week hunt=true.
  if (tier === "WINNERS_CONSOLATION_LADDER" || tier === "LOSERS_CONSOLATION_LADDER") return false;
 ...
 */
export function scoreIsSemiMoneyPlace(score, scores, path) {
  if (!score || score.phase !== "playoff") return false;
  if (scoreIsChampionshipHunt(score)) return false;
  const pts = Number(score.points);
  if (Number.isFinite(pts) && pts < PLAYOFF_STUB_POINTS) return false;
  const weeks = path || titlePathWeeks(scores, score.season);
  const { finalWeek, semiWeek, last } = weeks;
...
export function scoreIsPlayoffLineup(score, scores, path) {
  return scoreIsChampionshipHunt(score) || scoreIsSemiMoneyPlace(score, scores, path);
}
...
export function weekScoreBookFromTape(tape) {
  ...
  const pathBySeason = {};
  const pathFor = (season) => {
    const year = String(season || "");
    return pathBySeason[year] || (pathBySeason[year] = titlePathWeeks(scores, year));
  };
  const countableHunt = playoff.filter((s) => scoreIsPlayoffLineup(s, scores, pathFor(s && s.season)));
